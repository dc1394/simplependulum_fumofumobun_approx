...
namespace TAK_CameraController
{
    ...
    internal class CameraController : MonoBehaviour
    {
        ...
        private void MouseDragEvent(Vector3 mousePos)
        {
            ...
            if (Input.GetMouseButton((int)MouseButtonDown.MbdLeft))
            {
                // マウス左ボタンをドラッグした場合(なにもしない)
            }
            else if (Input.GetMouseButton((int)MouseButtonDown.MbdMiddle))
            {
                // マウス中ボタンをドラッグした場合(注視点の移動)
                this.CameraTranslate(-diff / 10.0f);
            }
            else if (Input.GetMouseButton((int)MouseButtonDown.MbdRight))
            {
        ...
        private void MouseEvent()
        {
            ...
            if (Input.GetMouseButtonDown((int)MouseButtonDown.MbdLeft) ||
            Input.GetMouseButtonDown((int)MouseButtonDown.MbdMiddle) ||
            Input.GetMouseButtonDown((int)MouseButtonDown.MbdRight))
            {
